...
def dense2sparse(prediction, rgb, param, viewPair, min_prob = 0.5, rayPool_thresh = 0, \
        enable_centerCrop = False, D_center = None, \
    ...
    inputs:
        ...
        param: np.float32(N_cubes,4): min_xyz & resol
    ...
    if enable_centerCrop:
        _Cmin, _Cmax = (D_orig-D_center)/2, (D_orig-D_center)/2 + D_center 
        ...
        param_new[:,:3] += param_new[:,3:4] * _Cmin # (N_cubes, 3) + (N_cubes,1) = (N_cubes, 3)
    ...
    for _n in range(N_cubes):
        if enable_rayPooling:
            ...
            rayPool_votes = rayPooling.rayPooling_1cube_numpy(cameraPOs, cameraTs, \
                    viewPair_viewIndx = viewPair[_n], param = param[_n],\
        ...
        if enable_rayPooling:
            rayPooling_votes_list.append(rayPool_votes[slc][vxl_ijk_tuple].astype(np.uint8)) # (D_center,)*3 --> (iN_voxel,)
...
def append_dense_2sparseList(prediction_sub, rgb_sub, param_sub, viewPair_sub, min_prob = 0.5, rayPool_thresh = 0, \
    ...
    inputs:
        ...
        param_sub: np.float32(N_cubes,4): min_xyz & resol
    ...
    cube_ijk_sub = param_sub[:,5:8].astype(np.uint16)
    param_sub = param_sub[:,:4].astype(np.float32) # only save min_xyz & resol
    ...
    sparse_output = dense2sparse(prediction = prediction_sub, rgb = rgb_sub, param = param_sub,\
            ...
            enable_centerCrop = True, D_center = D_center,\
    ...
    param_np = param_sub if param_np is None else np.vstack([param_np, param_sub])
...
def load_dense_as_sparse(files, D_center, min_prob=0.5, rayPool_thresh = 0):
    """
...
def filter_voxels(vxl_leftIndx_list=[],prediction_list=None, prob_thresh=None,\
        rayPooling_votes_list=None, rayPool_thresh=None):
    """
    thresholding using the prediction or rayPooling 
    consider the given vxl_leftIndx_list
    ...
    inputs:
        ...
        vxl_leftIndx_list[i]: np.bool (iN_voxels,)
    ---------
    outputs:
        vxl_leftIndx_list[i]: np.bool (iN_voxels,)
    """
    empty_vxl_leftIndx = True if len(vxl_leftIndx_list) == 0 else False
    if prediction_list is not None:
        ...
        for _c, _prediction in enumerate(prediction_list):
            ...
            if empty_vxl_leftIndx:
                vxl_leftIndx_list.append(_surf)
            else:
                vxl_leftIndx_list[_c] &= _surf
    empty_vxl_leftIndx = True if len(vxl_leftIndx_list) == 0 else False
    if rayPooling_votes_list is not None:
        ...
        for _cube, _votes in enumerate(rayPooling_votes_list):
            ...
            if empty_vxl_leftIndx:
                ...
                vxl_leftIndx_list[_cube] &= _surf
    return vxl_leftIndx_list


def save_sparseCubes_2ply(vxl_leftIndx_list, vxl_ijk_list, rgb_list, \
    ...
    outputs:
        save to .ply file
    """
    vxl_leftIndx_np = np.concatenate(vxl_leftIndx_list, axis=0) 
    N_voxels = vxl_leftIndx_np.sum()
    vxl_ijk_np = np.vstack(vxl_ijk_list)
    rgb_np = np.vstack(rgb_list)
    if not vxl_leftIndx_np.shape[0] == vxl_ijk_np.shape[0] == rgb_np.shape[0]:
    ...
    if normal_list is None:
        ...
        dt = np.dtype([('x', '<f4'), ('y', '<f4'), ('z', '<f4'), \
                ('nx', '<f4'), ('ny', '<f4'), ('nz', '<f4'), \
                ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])
        normal_np = np.vstack(normal_list)[vxl_leftIndx_np]
    saved_pts = np.zeros(shape=(N_voxels,), dtype=dt)

    # calculate voxels' xyz 
    xyz_list = []
    for _cube, _select in enumerate(vxl_leftIndx_list):
        resol = param[_cube][3]
        xyz_list.append(vxl_ijk_list[_cube][_select] * resol + param[_cube][:3][None,:]) # (iN, 3) + (1, 3)
    xyz_np = np.vstack(xyz_list)
    vxl_ijk_np, rgb_np = vxl_ijk_np[vxl_leftIndx_np], rgb_np[vxl_leftIndx_np]
...
def __debug():
    ...
    # outputs = load_dense_as_sparse(D_center=26, rayPool_thresh=0, min_prob=0.5,files=[file_path_lambda(_b) for _b in batch_range])
    ...
    # vxl_leftIndx_list = filter_voxels(vxl_leftIndx_list=[],prediction_list=prediction_list, prob_thresh=prob_thresh_list,\
    #         rayPooling_votes_list=rayPooling_votes_list, rayPool_thresh=5)
        
    # save_sparseCubes_2ply(vxl_leftIndx_list, vxl_ijk_list, rgb_list, \
