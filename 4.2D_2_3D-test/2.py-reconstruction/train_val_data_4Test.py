...
VGG_triplet_thresh = [None,0.5] # 6.5 ## if the two patches are too similiar, they are still be ignored. Because the baseline may be not large enough.
...
##    resol = np.float32(0.4*4.) #0.4 resolution / the distance between adjacent voxels
    cube_stride_over_Dsize = 1/2. ## cube_stride / cube_Dsize  1./.5 
    
    

temp_folder = '/home/mengqi/dataset/MVS/samplesVoxelVolume/modelfile_50x50x50_2D_2_3D_normal/temp_visualization/'
#visualizer = '/home/mengqi/pcl/pcl_program/tutorial/20160522_01_pcl_multi_visualizers_XYZRGBT_backup/build_qt/pcl_visualizer_demo'
visualizer = '/home/mengqi/pcl/pcl_program/tutorial/20160725_pcl_multi_visualizers_XYZRGBT_normal-backup/build-qt/pcl_visualizer_demo'
data_fld = '/home/mengqi/dataset/MVS/samplesVoxelVolume/modelfile_50x50x50_2D_2_3D/'
voxelVolume_txt_fld = '/home/mengqi/dataset/MVS/samplesVoxelVolume/pcl_txt_50x50x50_2D_2_3D_normal/'
camera_po_txt_fld = '/home/mengqi/dropbox/Dropbox/ust-ubuntu/MultiViewSegmentation_with_Prof.Liu/program/miniDataset/pos/'
model_imgs_fld = '/home/mengqi/dataset/MVS/Rectified_mean/'

pcd_folder = '/home/mengqi/dataset/MVS/samplesVoxelVolume/modelfile_50x50x50_2D_2_3D/saved_pcd/'
...
#----------------------------------------------------------------------
def get_VGG_triplet_Net_featureVec(modelIndx,xyz_np,view_set,patch2feature_fn,featurePair2simil_fn,hw_size):
    """
    ...
    # will subtract mean on GPU
    voxels = voxel_in_hull(modelIndx = modelIndx, datasetName = 'MVS', views_4_reconstr = view_set,\
                           MEAN_IMAGE_BGR = np.asarray([0,0,0]).astype(np.float32), patch_size=hw_size, xyz_np = xyz_np)
    ...
    for pt_batch_start_end_indx in voxels.iterate_candidates(voxels.candidates_valid_views, Max_img_patches = 3500):  
        ...
        if voxels.inputs.shape[0] == 0:
            filter_sub = np.zeros((voxels.NOofPts,)).astype(np.bool)
        else:
            output_feature_embed = patch2feature_fn(voxels.inputs) ## previous test_fn is defined in the VGG_Triplet.py
            #print("6it takes {:.3f}s".format(time.time() - start_time))
            (indx_n,indx_v) = np.where(voxels.valid_views)
            feature_dim = output_feature_embed.shape[1]
            feature_embed = np.zeros((voxels.NOofPts, voxels.N_views, feature_dim),dtype=np.float32) ## because the view indx starts from 1.
            ...
            dissimilarity = featurePair2simil_fn(feature_embed[:,voxels.viewPairs_dimIndx_array.flatten(),:].reshape([-1,feature_dim]))
            dissimilarity = dissimilarity.reshape([voxels.NOofPts, -1])

            #!!!!!!! 
            # big bug, output_feature_embed only calculate valid view patch's feature for each pt
            # then, when we calculate the dissimilarity/euclidian_dist between (v1, v2), 
            # by calculate the subtraction between -all- the pts' patch feature of view1 and view2
            # save it as dissimilarity / euclid_dist
            # SHOULD NOT thresholding using this value, because there are a lot patch features are 0.
            # should use voxels.valid_view_pairs to filter all the unvalid view pair' similiar values.
            dissimilarity *= voxels.valid_view_pairs
            #!!!!!!!
            ...
            filter_sub = voxels.select_xyzrgba(dissimilarity, threshold=threshold) ## diff_X_Euclid will be updated
            diff_X_Euclid_list.append(dissimilarity)
            #print("9it takes {:.3f}s".format(time.time() - start_time))
        desicion_onSurf_list.append(filter_sub)
    return np.vstack(diff_X_Euclid_list) if len(diff_X_Euclid_list) != 0 else None, \
            np.hstack(desicion_onSurf_list) if len(desicion_onSurf_list) != 0 else None
...
def RECONSTR_generate_3DSamples_param_np(modelIndx):
    ...
    x_=np.arange(-210,210,cube_stride).astype(np.float32) #+-300 
    y_=np.arange(-210,210,cube_stride).astype(np.float32) #+-300 
    z_=np.arange(500,800,cube_stride).astype(np.float32) #400,1000    / 600, 800
...
def RECONSTR_select_valid_pts_in_cube(predict, rgb_3D, param, VGGOccupancy, pt_norm = None, center_D_range = None):
    """
    ...
    # pts_onBoarderOfCube = [] # used to filter out the voxels, which are not in the center of the cube which we more believe in.
    for i in xrange(N_pts):
        # valid_pts_indx: {N_pts, predict.ndim}
        n, _, ix, iy, iz = valid_pts_indx[i]
        if center_D_range is not None: # only save the center part of the cube
            d_min, d_max = center_D_range
            if not (d_min <= ix <= d_max and d_min <= iy <= d_max and d_min <= iz <= d_max):
                # pts_onBoarderOfCube.append(i)
                continue
        x_min, y_min, z_min, resol, modelIndx = param[n][:5]
        x, y, z = x_min+ix*resol, y_min+iy*resol, z_min+iz*resol
        RGB = RGB_uint8[n,:,ix,iy,iz]

        _saved_pt = np.array((x, y, z)\
                + (() if pt_norm is None else tuple(pt_norm[n,:,ix,iy,iz]))\
                + tuple(RGB), dt)

        saved_pts = np.append(saved_pts, _saved_pt)
    ...
    return saved_pts #np.delete(xyz_rgba_np, pts_onBoarderOfCube, axis=0)
...
def visualize_N_densities_pcl(coloredCube_list, normalCube_list=None, density_list=None):
    ...
    files_2_visual=' '
    os.system('rm '+temp_folder+'*') # remember to clear the tmp_file_folder frequently    
