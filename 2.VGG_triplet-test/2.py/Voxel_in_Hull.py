...
class voxel_in_hull:
    def __init__(self, datasetName, modelIndx, MEAN_IMAGE_BGR, patch_size = 64):
        ...
        if datasetName == 'Tsinghua':
            ...
            self.d = 64 #8 # the distance between adjacent points
            x_=np.arange(-300,300,self.d) #400 2views_in_group
            y_=np.arange(-300,300,self.d)
            z_=np.arange(400,1000,self.d) #400,1000

        x,y,z = np.meshgrid(x_,y_,z_)
        ##### xyz1_voxel_candidate/xyz.shape = (N,4) because of homogeneous coordinate
        self.xyz1_voxel_candidate = np.concatenate((x.reshape((-1,1)),y.reshape((-1,1)),z.reshape((-1,1)),np.ones((x.size,1))),axis=1).astype(np.float32)
        ...
        if datasetName == 'Tsinghua':
            ...
            self.views_4_reconstr = range(1, 50)
            self.NOofViews = len(self.views_4_reconstr)
            ...
            self.view_pairs_array = np.asarray(self.view_pairs)
            self.NOofViewPairs = len(self.view_pairs)
            # view_pairs_array stores the indx of each view in the pairs
            self.view_indx_pairs_array = np.asarray([self.views_4_reconstr.index(self.view_pairs_array.reshape(-1,)[i]) 
        ...
        print(' NOofViews: {} \n views_4_reconstr: {} \n NOofViewPairs: {} \n view_pairs: {} \n color_filter: gray range thresh {:.3f} / hue variance thresh {:.3f}'.\
              format(self.NOofViews, self.views_4_reconstr, self.NOofViewPairs, self.view_pairs, \
    ...
    def load_imgs_MVS_meanIMG(self, modelIndx):
        ...
        rectified_img_folder = '/home/mengqi/dataset/MVS/Rectified_mean/scan'+str(modelIndx)+'/'
        file_list = os.listdir(rectified_img_folder)
        # if the viewIndx is start from 1, the 0th row of rectified_img_list = []
    ...
    def keep_voxel_in_hull(self):
        self.view_wh = np.zeros((self.xyz.shape[0], self.NOofViews*2), dtype=np.int) # store the image uv coordinates on each view for each 3d point
    ...
    def proj_pt_2_views(self, xyz1):
        view_wh = np.zeros((xyz1.shape[0], self.NOofViews, 3), dtype=np.int) 
        for i, viewIndx in enumerate(self.views_4_reconstr):
            # project all the 3D point in the xyz array to each view's camera plane, only preserve the ones
            # whose projection is in scope of that view
            ...
            view_wh[:,i,1:3] = perspective.astype(np.int)
            view_wh[:,i,0] =indx_inscope 
        return view_wh
    ...
    def keep_valid_view_pairs(self, view_wh):
        ...
        return pt_inScope_in_viewIndxes[:,self.view_indx_pairs_array[:,0]] & pt_inScope_in_viewIndxes[:,self.view_indx_pairs_array[:,1]]
    ...
    def get_valid_views(self, valid_view_pairs):
        ...
        valid_views = np.zeros((valid_view_pairs.shape[0], self.NOofViews)).astype(np.bool)
        for i in range(self.NOofViewPairs):
            view_pair_i = np.zeros(valid_views.shape).astype(np.bool)
            view1_indx = self.view_indx_pairs_array[i,0]
            view2_indx = self.view_indx_pairs_array[i,1]
    ...
    def generate_voxel_patches(self):
        ...
        self.inputs = np.zeros((self.xyz.shape[0] * self.NOofViews,3,self.patch_size,self.patch_size),\
        ...
        for i in range(0,self.xyz.shape[0]):
            ...
            for j, viewIndx in enumerate(self.viewIndxes):
                ...
                self.inputs[i*self.NOofViews + j] = patch
    ...
    def generate_patches_rgb_from_valid_views(self, iteration):
        ...
        self.inputs = np.zeros((NOofValidPatches,3,self.patch_size,self.patch_size),\
                          dtype=np.float32) # inputs[i][j]: the ith set jth view
        self.pt_rgb = np.zeros((self.NOofPts, self.NOofViews, 3), dtype=np.uint8)
    ...
    def iterate_minibatches(self, batchsize):
        ...
        for start_indx in range(0, self.xyz.shape[0] - batchsize + 1, batchsize):
            end_idx = start_indx
            yield self.inputs[self.NOofViews * start_indx: self.NOofViews * (start_indx + batchsize)], \
                  self.xyz[start_indx: start_indx + batchsize, :3]
        end_idx += batchsize
        yield self.inputs[self.NOofViews * end_idx: ], self.xyz[end_idx: , :3]
    ...
    def select_xyzrgba(self, diff_X_Euclid, threshold):
        diff_X_Euclid[diff_X_Euclid < 0.1] = threshold+100 # filter out the identical patches, in order to filter out the point out of the visual hull
        on_surf_decision = diff_X_Euclid < threshold
        ...
        filter = (on_surf_decision.sum(axis=1) > (self.valid_view_pairs.sum(axis=1) / 6.)).astype(np.bool) 
        # filter = (on_surf_decision.sum(axis=1) > 3).astype(np.bool) 
        
        # filter_color = self.color_filter(self.pt_rgb[:,self.view_indx_pairs_array[:,0],:], ~on_surf_decision, \
        ...
        pt_view_pair_rgb = self.pt_rgb[:,self.view_indx_pairs_array[:,0],:] 
        #pt_view_pair_rgb = (self.pt_rgb[:,self.view_indx_pairs_array[:,0],:] + self.pt_rgb[:,self.view_indx_pairs_array[:,1],:])/2
    ...
    def iterate_viewGroups(self):
        if self.datasetName == 'MVS':
            ...
            view_groups_valid = [v_i for v_i in self.view_groups_smaller if len(v_i)>=self.NOofViews]
            ...
            for view_group_selected in view_groups_valid:
                ...
                #indx_NOofviews = random.sample(range(0,len(view_group_selected)),self.NOofViews)
    ...
    def iterate_candidates(self, candidates_valid_views, Max_img_patches):
        ...
        end_idx = 0
        print('processing {} points: '.format(candidates_valid_views.shape[0]))
        while start_indx < candidates_valid_views.shape[0]:
            if start_indx == 0:
